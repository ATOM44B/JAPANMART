...
RANK2_RE = re.compile(r"ランク\s*([SABCD訳未])")
RANK_OPT_RE = re.compile(r"^(?:[SABCD]|訳|未)$")
SIZE_RE = re.compile(r"サイズ[：:]\s*\S+")
ID_PREFIX = {"komehyo": "k", "netmall": "n", "eearphone": "e"}
MODEL_BRAND = (("WH-", "SONY"), ("WF-", "SONY"), ("NW-", "SONY"), ("MDR-", "SONY"), ("IER-", "SONY"), ("SRS-", "SONY"),
               ("ATH-", "audio-technica"), ("AirPods", "APPLE"))
...
def find_price(text: str, pick: str = "last") -> int | None:
    m = YEN_RE.findall(text)
    if m:
        return to_int(m[0] if pick == "first" else m[-1])
...
def _clean_url(u: str, base: str) -> str | None:
    ...
    return None if BAD_IMG.search(full) else shopify_sized(full)


def shopify_sized(u: str) -> str:
    """Shopify's image server can resize on the fly. A 700 px picture is plenty on a phone and loads much faster."""
    if "cdn.shopify.com" not in u:
        return u
    if re.search(r"[?&]width=\d+", u):
        return re.sub(r"([?&]width=)\d+", r"\g<1>700", u)
    return u + ("&" if "?" in u else "?") + "width=700"
...
def model_key(title: str, brand: str, aliases: dict) -> str:
    ...
    toks = re.findall(r"[A-Za-z0-9][A-Za-z0-9.\-/]*", t)
    for i, tok in enumerate(toks):
        if len(tok) >= 3 and re.search(r"\d", tok) and not re.fullmatch(r"(19|20)\d\d", tok):
            key = tok.strip("-/.")
            if i > 0 and re.fullmatch(r"[A-Z]{1,4}", toks[i - 1]):  # "HD 800S", "IE 300"
                key = toks[i - 1] + " " + key
            return key
...
def parse_listing(html: str, page_url: str, site: dict, q: dict, aliases: dict) -> list[dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        price = find_price(text, site.get("price_pick", "last"))
        if not text or not price or SOLD_RE.search(text):
            continue
        seen.add(key)
        imgs = images_in(a, page_url) or (images_in(a.parent, page_url) if a.parent else [])
        rank = (RANK_RE.search(text) or RANK2_RE.search(text) or [None, ""])[1]
        store = (STORE_RE.search(text) or [None, ""])[1].strip()
        if not store:
            br = re.findall(r"【([^】]+)】", text)
            if br and re.fullmatch(r"[\u3040-\u30ff\u4e00-\u9fff]{2,6}", br[-1]) and br[-1] not in ("中古", "新品"):
                store = br[-1]
        if "ランク" in text:
            head = re.split(r"\s*(?:中古)?ランク[：:\s]", text)[0]
        else:
            head = re.sub(r"[¥￥]?\s*[\d,]{3,9}\s*(?:円|\(税込\)|（税込）)", " ", text)
        title = dedupe_title(re.sub(r"\s+", " ", SIZE_RE.sub(" ", re.sub(r"【(?:中古|新品)】", " ", head))).strip())
        brand = detect_brand(title, q.get("brand", ""), aliases)
        if not brand or brand == title.split()[0][:20]:
            brand = next((b for pre, b in MODEL_BRAND if pre in title), brand)
        cat = q["cat"]
...
def parse_shopify(payload, base: str, q: dict, aliases: dict) -> list[dict]:
    """Items from a Shopify collection feed (…/products.json). Photos, prices and stock come as data, no page reading needed."""
    out: list[dict] = []
    for p in (payload.get("products") if isinstance(payload, dict) else None) or []:
        handle = p.get("handle")
        avail = [v for v in (p.get("variants") or []) if v.get("available", True)]
        prices = []
        for v in avail:
            try:
                x = float(v.get("price") or 0)
            except (TypeError, ValueError):
                x = 0
            if x > 0:
                prices.append(x)
        raw = str(p.get("title") or "")
        if not handle or not prices or SOLD_RE.search(raw):
            continue
        br = re.findall(r"【([^】]+)】", raw)
        store = br[-1] if br and re.fullmatch(r"[\u3040-\u30ff\u4e00-\u9fff]{2,6}", br[-1]) and br[-1] not in ("中古", "新品") else ""
        title = re.sub(r"\s+", " ", re.sub(r"【[^】]*】", " ", raw)).strip()
        rank = ""
        for v in avail:
            rank = next((str(v.get(k)).strip() for k in ("option1", "option2", "option3", "title") if RANK_OPT_RE.match(str(v.get(k) or "").strip())), "")
            if rank:
                break
        if not rank:
            tags = p.get("tags") or []
            blob = (" ".join(tags) if isinstance(tags, list) else str(tags)) + " " + re.sub(r"<[^>]+>", " ", str(p.get("body_html") or ""))[:800]
            m = RANK2_RE.search(blob)
            rank = m.group(1) if m else ""
        imgs = []
        for im in p.get("images") or []:
            src = str(im.get("src") or "") if isinstance(im, dict) else ""
            src = "https:" + src if src.startswith("//") else src
            if src.startswith("http") and not BAD_IMG.search(src):
                imgs.append(shopify_sized(src))
        brand = str(p.get("vendor") or "").strip() or detect_brand(title, q.get("brand", ""), aliases)
        cat = q["cat"]
        model = model_key(title, brand, aliases)
        out.append({"id": ID_PREFIX.get(q["site"], "x") + re.sub(r"[^A-Za-z0-9]", "", str(p.get("id") or handle)),
                    "cat": cat, "src": q["site"], "brand": brand, "model": model, "desc": title[:90], "cond": rank,
                    "jpy": int(min(prices)), "kg": KG.get(cat, 0.8), "sq": model, "store": store,
                    "imgs": imgs[:4], "snap": False, "url": f"{base}/products/{handle}"})
    return out


def discover_collections(html: str, page_url: str, mapping: dict) -> dict:
    """Read the shop's own menu: find the links whose text starts with one of the wanted category names."""
    soup = BeautifulSoup(html, "lxml")
    found: dict = {}
    for a in soup.find_all("a", href=True):
        text = re.sub(r"\s+", "", a.get_text(" ", strip=True))
        ...
        for label, cat in mapping.items():
            if text.startswith(label):
                found.setdefault(href.split("?")[0].rstrip("/"), (cat, label))
    return found


def expand_queries(cfg: dict, fetcher, qlog: list) -> list[dict]:
    out: list[dict] = []
    for q in cfg["queries"]:
        if "discover" not in q:
            out.append(q)
            continue
        html, note = fetcher.get(q["discover"])
        found = discover_collections(html, q["discover"], q["map"]) if html else {}
        qlog.append({"site": q["site"], "cat": "discover", "url": q["discover"], "found": len(found), "note": "" if html else note})
        if not found:
            found = {f["url"]: (f["cat"], f["q"]) for f in q.get("fallback", [])}
        keep = {k: v for k, v in q.items() if k not in ("discover", "map", "fallback")}
        for url, (cat, label) in found.items():
            out.append({**keep, "cat": cat, "url": url, "q": label})
    return out


def collect_shopify(fetcher, q: dict, site: dict, cfg: dict, counts: dict, qlog: list, streak: dict, aliases: dict) -> list[dict]:
    """One collection: the JSON feed first (photos and prices as data); the normal page if the feed is not available."""
    base = site["base"].rstrip("/")
    coll = q["url"].split("?")[0].rstrip("/")
    cap, pages, got_all = int(q.get("max_items", 150)), int(q.get("pages", 2)), []
    for pg in range(1, pages + 1):
        entry = {"site": q["site"], "cat": q["cat"], "url": f"{coll}/products.json?limit=250&page={pg}", "found": 0, "note": ""}
        qlog.append(entry)
        html, note = fetcher.get(entry["url"])
        payload = None
        if html:
            try:
        ...
        if isinstance(payload, dict) and isinstance(payload.get("products"), list):
            got = parse_shopify(payload, base, q, aliases)
            entry["found"] = len(got)
            more = len(payload["products"]) >= 250
        else:
            entry["note"] = f"feed not available ({note if not html else 'not JSON'}), reading the page instead"
            purl = f"{coll}?page={pg}"
            html, note = fetcher.get(purl)
            entry = {"site": q["site"], "cat": q["cat"], "url": purl, "found": 0, "note": "" if html else note}
            qlog.append(entry)
            if not html:
                counts[q["site"]]["failed"] += 1
                streak[q["site"]] += 1
                break
            got = parse_listing(html, purl, site, q, aliases)
            entry["found"] = len(got)
            more = len(got) >= 10
        streak[q["site"]] = 0
        counts[q["site"]]["ok"] += 1
        got_all += got
        if not more or len(got_all) >= cap:
            break
    return got_all[:cap]
...
def run(cfg: dict, fetcher, data_dir: Path = DATA, dry_run: bool = False, limit: int | None = None, fx_fn=fetch_fx) -> dict:
    ...
    aliases = cfg.get("brand_alias", {})
    for q in expand_queries(cfg, fetcher, qlog):
        site = sites[q["site"]]
        lo, hi = q.get("min_jpy", cfg.get("min_jpy", 0)), q.get("max_jpy", cfg.get("max_jpy", 10**9))
        if streak[q["site"]] >= 3 and counts[q["site"]]["ok"] == 0:  # refused the first 3 requests: stop asking
            ...
            continue
        if site.get("type") == "shopify" and q.get("url"):
            for it in collect_shopify(fetcher, q, site, cfg, counts, qlog, streak, aliases):
                if lo <= it["jpy"] <= hi:
                    items.setdefault(it["id"], it)
            continue
        taken = 0
        for pg in range(1, int(q.get("pages", pages)) + 1):
            if budget <= 0:
                break
            url = q.get("url") or q.get("url_tpl", site["search_url"]).format(q=quote(q["q"]))
            if pg > 1:
                names = site.get("page_param", "page")
                names = names if isinstance(names, list) else [names]
                url += ("&" if "?" in url else "?") + "&".join(f"{n}={pg}" for n in names)
            budget -= 1
            ...
            streak[q["site"]] = 0
            counts[q["site"]]["ok"] += 1
            got = parse_listing(html, url, site, q, aliases)
            entry["found"] = len(got)
            for it in got:
                if lo <= it["jpy"] <= hi and taken < int(q.get("max_items", 10**6)):
                    items.setdefault(it["id"], it)
                    taken += 1
            if len(got) < 10 or taken >= int(q.get("max_items", 10**6)):  # last page, or enough from this search
                break
        if budget <= 0:
            break
    ...
    # Japan used-market reference per model: median asking price of all listings of the same model, from every source.
    groups: dict = {}
    for it in live:
    ...
    def deal(it):  # how far below the typical used price of its model
        return it["mkt"] / it["jpy"] if it["mktN"] > 1 else 1.0

    # Balanced set: no single shop may fill the whole app. Over the cap, the best deals stay.
    caps = cfg.get("source_caps", {})
    kept: list = []
    for src in sites:
        rows = [i for i in live if i["src"] == src]
        cap = int(caps.get(src, 10**9))
        if len(rows) > cap:
            rows.sort(key=lambda i: (deal(i), i["jpy"]), reverse=True)
            rows = rows[:cap]
        kept += rows
        counts[src]["items"] = len(rows)
    live = kept
    ...
    todo += sorted((i for i in live if i["imgs"] and len(i["imgs"]) < 2 and i["id"] not in prev_imgs and i["src"] != "eearphone"), key=deal, reverse=True)
